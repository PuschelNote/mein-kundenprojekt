...
export default async function BestellungenPage() {
  ...
  const [{ tische, gerichte }, bestellungen] = await Promise.all([
    ...
    <section className="order-list">
      ...
      {bestellungen.length === 0 ? <div className="empty-state">Noch keine Bestellungen vorhanden.</div> : bestellungen.map((bestellung) => {
        ...
        return <article className={`order-card ${bestellung.status}`} key={bestellung.id}>
          ...
          {bestellung.status === "offen" ? <details><summary>Positionen bearbeiten</summary><BestellungForm action={updateBestellungAction} tische={tische} gerichte={gerichte} bestellung={{ id: bestellung.id, tischId: bestellung.tischId, positionen: bestellung.positionen.map((position) => ({ gerichtId: position.gerichtId, menge: position.menge, sonderwunsch: position.sonderwunsch ?? "" })) }} /></details> : null}
