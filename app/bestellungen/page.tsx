...
const statusLabel = { offen: "In Zubereitung", zubereitet: "Abholbereit", serviert: "Serviert", bezahlt: "Bezahlt", storniert: "Storniert" } as const;
    ...
    <section className="order-list">
      ...
      {bestellungen.length === 0 ? <div className="empty-state">Noch keine Bestellungen vorhanden.</div> : bestellungen.map((bestellung) => {
        ...
        return <article className={`order-card ${bestellung.status}`} key={bestellung.id}>
          <div className="dish-heading"><h3>Tisch {bestellung.tisch.nummer}</h3><span className="reservation-status">{statusLabel[bestellung.status]}</span></div>
          {bestellung.status === "zubereitet" ? <p className="order-ready-notice" role="status"><strong>Essen ist fertig.</strong> Bitte in der Küche abholen und anschließend als serviert markieren.</p> : null}
          ...
          {bestellung.status === "offen" || bestellung.status === "zubereitet" || bestellung.status === "serviert" ? <BestellungStatusForm action={updateBestellungStatusAction} id={bestellung.id} status={bestellung.status} /> : null}
